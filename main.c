...
//BF_20221120_REMOVE_DEAD #define PAYLOAD_END_SIGNATURE        0xDEAD
#define PAYLOAD_END_SIGNATURE_SZ     (2U)

/*
|--------------------------------------------------------------------------------------------
|      common        |   dpp_cmd_header , dpp_cmd_decimation    |     dpp_cmd_header , algo |
...
void print_to_file(char  * file_name, uint8_t * input, size_t sz) {
	FILE* fptr = fopen(file_name, "wb");
	fwrite(input, 1, sz, fptr);
	fclose(fptr);
}
...
void* sample_code_on_cpu() {
	...
	size_t mem_size = sizeof(struct dpp_common) +
		sizeof(struct dpp_cmd_decimation); //+ PAYLOAD_END_SIGNATURE_SZ;
	...
	common->payload_size = sizeof(struct dpp_cmd_decimation);
	...
	//decimation->frame_width = 848U;
	//decimation->frame_height = 480U;

	decimation->frame_width = 128U;
	decimation->frame_height = 128U;

	decimation->scale_factor = 4U;

	offset += decimation->header.cmd_size;


	//calculate the cs
	common->byte_sum = cs(u8_p_begin_of_payload, common->payload_size);//test1
	printf("\nPAYLOAD_CS: 0x%02X\n", common->byte_sum);

	//offset += new_algo->header.cmd_size; BF_20221120_REMOVE_DEAD
	//*(((uint16_t*)((uint8_t*)payload_buffer + offset))) = PAYLOAD_END_SIGNATURE;

	//print_to_file("algo_ctrl.bin", (uint8_t*)payload_buffer, mem_size);
	print_to_file("algo_ctrl_128x128.bin", (uint8_t*)payload_buffer, mem_size);
...
/// </summary>
/// <returns></returns>
void* sample_code_on_cpu_temporal() {
	uint8_t* u8_p_begin_of_payload = NULL;
	size_t mem_size = sizeof(struct dpp_common) +
	...
	common->ver.major = DPP_VERSION_MAJOR;
	common->ver.minor = DPP_VERSION_MINOR;
	common->payload_size = sizeof(sizeof(struct dpp_cmd_temporal));
	common->cycle_counter = 0U;
	...
	//temporal->frame_width = 1280U;
	//temporal->frame_height = 960U;
	temporal->frame_width = 128U;
	temporal->frame_height = 128U;
	temporal->alpha = 0.4F;
	temporal->delta = 23U;
...
//BF_20221120_INVALID_BYTESUM
void wrong_cs_handler(uint8_t cs) {
	printf("Wrong checksum %u\n", cs);
}

void print_msg_array_u8(uint8_t * msg, size_t sz) {
	rsize_t i = 0;
	printf("\nprint_msg_array_u8:\n");
	for (i = 0; i < sz; i++) {
		printf("%u ", msg[i]);
...
bool sample_code_on_dsp(void* payload_buffer) {
	bool res = false;
	struct dpp_common* common = (struct dpp_common*)payload_buffer;
	...
	//BF_20221120_INVALID_BYTESUM
	uint8_t* u8_p_begin_of_payload = (uint8_t*)payload_buffer + offset;
	if (cs(u8_p_begin_of_payload, common->payload_size) == common->byte_sum) {
		
		res = true;
	}
	else {
		wrong_cs_handler(common->byte_sum);
	}
	...
	if (true == res) {
		...
		while (offset < common->payload_size + begin) {
			...
			case DPP_TEMPORAL_ALGO_ID:
				...
				printf("alpha        : %f \n", temporal->alpha); //BF_20221120_DELTA_ALPHA_SWAP
				printf("delta        : %u \n", temporal->delta);
				break;
	...
	//print_msg_array_u8((uint8_t*)payload_buffer, 40);
...
int main(int argc, char* argv[]) {
	...
	void* payload_buffer = sample_code_on_cpu_3algo();
